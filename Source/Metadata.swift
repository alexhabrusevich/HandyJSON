            ...
            for i in 0..<self.numberOfFields {
                ...
                guard let mangledTypeName = fieldRecords[i].mangledTypeName else { continue }
                               let functionMap: [String: () -> Any.Type?] = [
                                   "function": { _getTypeByMangledNameInContext(mangledTypeName, getMangledTypeNameSize(mangledTypeName), genericContext: self.contextDescriptorPointer, genericArguments: self.genericArgumentVector) }
                               ]
